                                ...
                                if (msg.includes('SystemInformation:')) {
                                    ...
                                    if (config.censor_ntr) {
                                        ...
                                        if (censor_result !== '无ntr') {
                                            ...
                                            if(censor_result["ntr类型"]==="1") {
                                                if (config.groupmessage_withId){
                                                    content = config.censor_ntr_replace_name.replace('<userName>', userName) + '(id:' + authID + '):' + message
                                                }else{
                                                    content = config.censor_ntr_replace_name.replace('<userName>', userName)  + ':' + message
                                                }
                                            }else{
                                                if (config.groupmessage_withId){
                                                    content = userName+ '(id:' + authID + '):' + `******(一些不好的话，被${config.censor_ntr_name[0]}隐藏了)`
                                                }else{
                                                    content = userName  + ':' + `******(一些不好的话，被${config.censor_ntr_name[0]}隐藏了)`
                                                }                                                
                                            }
                                            logger.info(`用户${authID}触发ntr审查，已替换为：${content}`)
                                        }                                        
                                    } 
