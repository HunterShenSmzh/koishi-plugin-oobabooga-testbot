...
exports.Config = koishi_1.Schema.intersect([
    ...
    koishi_1.Schema.object({
        ...
        visual_module: koishi_1.Schema.boolean()
            .description('是否开启视觉总控')
            .default(false),
        Multimodel: koishi_1.Schema.boolean()
            .description('是否使用模型本身的多模态（需要模型支持多模态，启动后会覆盖外置视觉模式识别，TGW，ollama，Deepseek接口不可用）')
            .default(false),
    ...
    koishi_1.Schema.object({
        ...
        censor_ntr: koishi_1.Schema.boolean()
            .description('是否开启模型内部ntr审查，检查是否有人冒充主人（真群聊模式下使用）')
            .default(false),
...
async function createRequest(config, session, customConfig = {}, apiMode) {
    ...
    if (api_select == "TGW") {
        ...
        let requestBody = Object.assign({}, defaultConfig, NewcustomConfig);
        let attempt = 0;
        while (attempt <= maxRetries) {
            try {
                let response = await axios.post(url, requestBody, { timeout: requestTimeout });
                if (response.status == 200) {
                    //cot处理
                    response = await handleCoT(response, config, session, "TGW");
                    return response;
                } else {
                    console.log(`TGW API request failed with status ${response.status} on attempt ${attempt + 1}/${maxRetries + 1}`);
                    if (attempt >= maxRetries) {
                        session.send(`TGW API请求失败，状态码：${response.status}。已达最大重试次数。`);
                        return;
                    }
                }
            } catch (error) {
                const isTimeout = error.code === 'ECONNABORTED' || (error.message && error.message.toLowerCase().includes('timeout'));
                console.log(`TGW API request attempt ${attempt + 1}/${maxRetries + 1} failed. Error: ${error.message}${isTimeout ? ' (Timeout)' : ''}`);
                if (attempt >= maxRetries) {
                    const errorMessage = error.response ? JSON.stringify(error.response.data, null, 2) : error.message;
                    session.send(`TGW API请求最终失败: ${errorMessage}`);
                    console.log('TGW API请求最终失败:', error.response ? error.response.data : error.message);
                    return;
                }
                session.send(`TGW API请求失败，正在尝试重试 (${attempt + 1}/${maxRetries + 1})...`);
            }
            attempt++;
            if (attempt <= maxRetries) {
    ...
    if (api_select == "DeepSeek") {
        let url
        let lastMessageContentForCompletion = ''
        //不续写
        ...
        if (NewcustomConfig.continue_ == true) {
            ...
            if (NewcustomConfig.messages && NewcustomConfig.messages.length > 0) {
                // 设置 prefix 参数为 true
                lastMessageContentForCompletion = NewcustomConfig.messages[NewcustomConfig.messages.length - 1];
                lastMessageContentForCompletion.prefix = true;
            }
        }
        //去掉continue_
        ...
        while (attempt <= maxRetries) {
            try {
                let response = await axios.post(url, requestBody, {
                    headers: { 'Authorization': `Bearer ${config.apiKey_Deepseek}` },
                    timeout: requestTimeout
                });
                if (response.status == 200) {
                    if (lastMessageContentForCompletion && lastMessageContentForCompletion.content) {
                        response.data.choices[0].message.content = lastMessageContentForCompletion.content + response.data.choices[0].message.content;
                    }
                    if (config.deepseek_r1_distill) {
                        if (response.data.choices[0].message.reasoning_content) {
                            if (config.deepseek_r1_distill_send_think) {
                    ...
                    console.log(`DeepSeek API request failed with status ${response.status} on attempt ${attempt + 1}/${maxRetries + 1}`);
                    if (attempt >= maxRetries) {
                        session.send(`DeepSeek API请求失败，状态码：${response.status}。已达最大重试次数。`);
                        return;
                    }
                }
            } catch (error) {
                const isTimeout = error.code === 'ECONNABORTED' || (error.message && error.message.toLowerCase().includes('timeout'));
                console.log(`DeepSeek API request attempt ${attempt + 1}/${maxRetries + 1} failed. Error: ${error.message}${isTimeout ? ' (Timeout)' : ''}`);
                if (attempt >= maxRetries) {
                    const errorMessage = error.response ? JSON.stringify(error.response.data, null, 2) : error.message;
                    session.send(`DeepSeek API请求最终失败: ${errorMessage}`);
                    console.log('DeepSeek API请求最终失败:', error.response ? error.response.data : error.message);
                    return;
                }
                session.send(`DeepSeek API请求失败，正在尝试重试 (${attempt + 1}/${maxRetries + 1})...`);
            }
            attempt++;
            if (attempt <= maxRetries) {
                await new Promise(resolve => setTimeout(resolve, retryDelay));
    ...
    if (api_select == "OpenAI") {
            ...
            if (NewcustomConfig.messages && NewcustomConfig.messages.length > 0) {
                ...
                if (lastMessageObject.role === 'assistant') {
                    if (Array.isArray(lastMessageObject.content)) {
                        lastMessageContentForCompletion = lastMessageObject.content.filter(part => part.type === 'text')
                            .map(part => part.text)
                            .join('');
                    } else if (typeof lastMessageObject.content === 'string') {
                        lastMessageContentForCompletion = lastMessageObject.content;
                    } else {
                        console.warn("Unexpected last message content format:", lastMessageObject.content);
                    }
                }
                let NewmessageText = convertDialogue(NewcustomConfig);
                NewcustomConfig.messages = [
                    { "role": "system", "content": "请体会对话中的人设与要求，按照要求进行续写" },
                    { "role": "user", "content": `请对以下文本进行续写，你应该只输出末尾assistant的续写内容，注意只输出续写，不要对文本进行复述：${NewmessageText}` }
                ];
            }
        }
        //去掉continue_
        ...
        while (attempt <= maxRetries) {
            try {
                let axiosConfig = {
                    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${config.apiKey_OpenAI}` },
                    timeout: requestTimeout
                };
                if (agent) {
                    axiosConfig.httpsAgent = agent;
                }
                let response = await axios.post(url, requestBody, axiosConfig);
                if (response.status == 200) {
                    if (lastMessageContentForCompletion !== '') {
                        response.data.choices[0].message.content = lastMessageContentForCompletion + response.data.choices[0].message.content;
                    }
                    response = await handleCoT(response, config, session, "OpenAI");
                    return response;
                } else {
                    console.log(`OpenAI API request failed with status ${response.status} on attempt ${attempt + 1}/${maxRetries + 1}`);
                    if (attempt >= maxRetries) {
                        session.send(`OpenAI API请求失败，状态码：${response.status}。已达最大重试次数。`);
                        return;
                    }
                }
            } catch (error) {
                const isTimeout = error.code === 'ECONNABORTED' || (error.message && error.message.toLowerCase().includes('timeout'));
                console.log(`OpenAI API request attempt ${attempt + 1}/${maxRetries + 1} failed. Error: ${error.message}${isTimeout ? ' (Timeout)' : ''}`);
                if (attempt >= maxRetries) {
                    const errorMessage = error.response ? JSON.stringify(error.response.data, null, 2) : error.message;
                    session.send(`OpenAI API 请求最终失败: ${errorMessage}`);
                    console.log('OpenAI API 请求最终失败:', error.response ? error.response.data : error.message);
                    return;
                }
                session.send(`OpenAI API 请求失败，正在尝试重试 (${attempt + 1}/${maxRetries + 1})...`);
            }
            attempt++;
            if (attempt <= maxRetries) {
                await new Promise(resolve => setTimeout(resolve, retryDelay));
    ...
    if (api_select == "OpenAIPlus") {
        ...
        while (attempt <= maxRetries) {
            try {
                let axiosConfig = {
                    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${config.apiKey_OpenAIPlus}` },
                    timeout: requestTimeout
                };
                if (agent) {
                    axiosConfig.httpsAgent = agent;
                ...
                if (response.status == 200) {
                    ...
                    console.log(`OpenAIPlus API request failed with status ${response.status} on attempt ${attempt + 1}/${maxRetries + 1}`);
                    if (attempt >= maxRetries) {
                        session.send(`OpenAIPlus API请求失败，状态码：${response.status}。已达最大重试次数。`);
                        return;
                    }
                }
            } catch (error) {
                const isTimeout = error.code === 'ECONNABORTED' || (error.message && error.message.toLowerCase().includes('timeout'));
                console.log(`OpenAIPlus API request attempt ${attempt + 1}/${maxRetries + 1} failed. Error: ${error.message}${isTimeout ? ' (Timeout)' : ''}`);
                if (attempt >= maxRetries) {
                    const errorMessage = error.response ? JSON.stringify(error.response.data, null, 2) : error.message;
                    session.send(`OpenAIPlus API 请求最终失败: ${errorMessage}`);
                    console.log('OpenAIPlus API 请求最终失败:', error.response ? error.response.data : error.message);
                    return;
                }
                session.send(`OpenAIPlus API 请求失败，正在尝试重试 (${attempt + 1}/${maxRetries + 1})...`);
            }
            attempt++;
            if (attempt <= maxRetries) {
                await new Promise(resolve => setTimeout(resolve, retryDelay));
        ...
        while (attempt <= maxRetries) {
            try {
                ...
                if (response.status == 200) {
                    ...
                    console.log(`${api_select} API request failed with status ${response.status} on attempt ${attempt + 1}/${maxRetries + 1}`);
                    if (attempt >= maxRetries) {
                        session.send(`${api_select} API请求失败，状态码：${response.status}。已达最大重试次数。`);
                        return;
                    }
                }
            } catch (error) {
                const isTimeout = error.code === 'ECONNABORTED' || (error.message && error.message.toLowerCase().includes('timeout'));
                console.log(`${api_select} API request attempt ${attempt + 1}/${maxRetries + 1} failed. Error: ${error.message}${isTimeout ? ' (Timeout)' : ''}`);
                if (attempt >= maxRetries) {
                    const errorMessage = error.response ? JSON.stringify(error.response.data, null, 2) : error.message;
                    session.send(`${api_select} API 请求最终失败: ${errorMessage}`);
                    console.log(`${api_select} API 请求最终失败:`, error.response ? error.response.data : error.message);
                    return;
                }
                session.send(`${api_select} API 请求失败，正在尝试重试 (${attempt + 1}/${maxRetries + 1})...`);
            }
            attempt++;
            if (attempt <= maxRetries) {
                await new Promise(resolve => setTimeout(resolve, retryDelay));
    ...
    if (api_select == "Ollama") {
        let url = prepareURL(config, api_select);
        //续写
        let lastMessageOriginalContent = '';
        if (NewcustomConfig.continue_ == true) {
            if (NewcustomConfig.messages && NewcustomConfig.messages.length > 0) {
                const lastMsgObj = NewcustomConfig.messages[NewcustomConfig.messages.length - 1];
                if (lastMsgObj && typeof lastMsgObj.content === 'string') {
                    lastMessageOriginalContent = lastMsgObj.content;
                }
            }
        }
        //去掉continue_
        ...
        let requestBody = defaultConfig;
        let attempt = 0;
        while (attempt <= maxRetries) {
            try {
                let response = await axios.post(url, requestBody, { timeout: requestTimeout });
                if (response.status == 200) {
                    if (lastMessageOriginalContent !== '') {
                        response.data.message.content = lastMessageOriginalContent + response.data.message.content;
                    }
                    ...
                    console.log(`Ollama API request failed with status ${response.status} on attempt ${attempt + 1}/${maxRetries + 1}`);
                    if (attempt >= maxRetries) {
                        session.send(`Ollama API请求失败，状态码：${response.status}。已达最大重试次数。`);
                        return;
                    }
                }
            } catch (error) {
                const isTimeout = error.code === 'ECONNABORTED' || (error.message && error.message.toLowerCase().includes('timeout'));
                console.log(`Ollama API request attempt ${attempt + 1}/${maxRetries + 1} failed. Error: ${error.message}${isTimeout ? ' (Timeout)' : ''}`);
                if (attempt >= maxRetries) {
                    const errorMessage = error.response ? JSON.stringify(error.response.data, null, 2) : error.message;
                    session.send(`Ollama API 请求最终失败: ${errorMessage}`);
                    console.log('Ollama API 请求最终失败:', error.response ? error.response.data : error.message);
                    return;
                }
                session.send(`Ollama API 请求失败，正在尝试重试 (${attempt + 1}/${maxRetries + 1})...`);
            }
            attempt++;
            if (attempt <= maxRetries) {
                await new Promise(resolve => setTimeout(resolve, retryDelay));
    ...
    if (api_select == "TabbyAPI") {
        ...
        while (attempt <= maxRetries) {
            try {
                let axiosConfig = {
                    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${config.apiKey_TabbyAPI}` },
                    timeout: requestTimeout
                };
                let response = await axios.post(url, requestBody, axiosConfig);
                if (response.status == 200) {
                    ...
                    console.log(`TabbyAPI API request failed with status ${response.status} on attempt ${attempt + 1}/${maxRetries + 1}`);
                    if (attempt >= maxRetries) {
                        session.send(`TabbyAPI API请求失败，状态码：${response.status}。已达最大重试次数。`);
                        return;
                    }
                }
            } catch (error) {
                const isTimeout = error.code === 'ECONNABORTED' || (error.message && error.message.toLowerCase().includes('timeout'));
                console.log(`TabbyAPI API request attempt ${attempt + 1}/${maxRetries + 1} failed. Error: ${error.message}${isTimeout ? ' (Timeout)' : ''}`);
                if (attempt >= maxRetries) {
                    const errorMessage = error.response ? JSON.stringify(error.response.data, null, 2) : error.message;
                    session.send(`TabbyAPI API 请求最终失败: ${errorMessage}`);
                    console.log('TabbyAPI API 请求最终失败:', error.response ? error.response.data : error.message);
                    return;
                }
                session.send(`TabbyAPI API 请求失败，正在尝试重试 (${attempt + 1}/${maxRetries + 1})...`);
            }
            attempt++;
            if (attempt <= maxRetries) {
                await new Promise(resolve => setTimeout(resolve, retryDelay));
...
async function Alarmrecall(session, config, sessionId, character) {
    ...
    input.push({
        "role": "assistant", "content": `当前时间：${Time}
预定的提醒时间到了
我应该发送提醒消息：
时间` });
    //准备request
        ...
        if (match[1] !== "None") {
            ...
            if (files.length > 0) {
                ...
                if (fs.existsSync(filePath)) {
                    ...
                    await session.send(`<img src="data:image/${fileExtension};base64,${base64Image}"/>`);
                } else {
...
async function censor_ntr(config, session, content, authID, userName, message) {
    ...
    if (config.censor_ntr_whitelist_id.length === 0 && config.censor_ntr_name.length === 0) {
        ...
        if (config.censor_ntr_name.includes(userName)) {
            console.log(`用户${authID}不在白名单内，夹了`)
        ...
        if (config.censor_ntr_fast_mode) {
            ...
            if (!containsMasterName) {
                console.log('快速模式：用户名不包含任何主人名字，跳过ntr审查');
    ...
    try {
        censor_data = JSON.parse(censor_data);
        console.log('ntr分析内容：', censor_data)
...
async function sendText(session, output, config, ctx, authID) {
    ...
    } else if (config.memory_table) {
        console.log(`记忆表功能已在配置中启用，但 ctx.memorytable 不可用。ctx.memorytable 的值:`, ctx.memorytable);
    }
            ...
            return new Promise(async (resolve, reject) => {
                ...
                channelTasks[gchannelId].push(async () => {
                    try {
                        ...
                        // 获取记忆表
                        if (config.memory_table && ctx.memorytable) {
                            const { trait, memory_st, memory_lt, traits, kbs } = await ctx.memorytable.getMem(session.userId, session.channelId.toString().replace(/-/g, ''), session);
                            fullinput.forEach(entry => {
                                if (entry.role === 'system') {
                                    if (traits) {
                                        let tmpContent = `这里是你回想起群友们的信息：\n` + JSON.stringify(traits)
                                        entry.content = entry.content + tmpContent;
                                    } else {
                                        if (trait) {
                                            let tmpContent = `这里是你回想起${config.groupmessage_withId ? userName + '(id:' + session.author.id + ')' : userName}的信息：\n` + JSON.stringify(trait)
                                            entry.content = entry.content + tmpContent;
                                        } else {
                                            console.log(`未回想起特征信息：因为trait为空`)
                                        }
                                    }
                                    if (memory_st) {
                                        let tmpContent = `这里是最近群里聊天内容的总结：\n` + JSON.stringify(memory_st)
                                        entry.content = entry.content + tmpContent;
                                    }
                                    if (memory_lt) {
                                        let tmpContent = `这里是你对本群的长期记忆：\n` + JSON.stringify(memory_lt)
                                        entry.content = entry.content + tmpContent;
                                    }
                                    if (kbs) {
                                        let tmpContent = `这里是一些相关的背景知识：\n` + JSON.stringify(kbs)
                                        entry.content = entry.content + tmpContent;
                                    }
                                    // 在system promt后面插入当前的日期、星期和时间
                                    let tmpContent = `\n当前时间：${new Date().getFullYear()}年${new Date().getMonth() + 1}月${new Date().getDate()}日 ${new Date().getHours()}:${new Date().getMinutes()} 星期${'日一二三四五六'.charAt(new Date().getDay())}`
                                    entry.content = entry.content + tmpContent;
                                }
                            });
                        } else if (config.memory_table) {
    ...
    ctx.middleware(async (session, next) => {
        try {
            ...
            if (ctx.bots[session.uid])
                return;
            // @触发
            if (config.if_at && session.elements.some(el => el.type === "at" && ((el.attrs.id === session.bot.user.id) || (el.attrs.name === session.bot.user.name)))) {
