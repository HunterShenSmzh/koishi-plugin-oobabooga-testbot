...
### 新版本新增：
修复用户输入与模型收到内容不符的底层逻辑错误，标准化模型输出，防止部分HTML代码无法输出。<br>
新增模块：视觉模块beta测试，oob.vision指令，表情包模块<br>
...
async function ImgRequest(config,task_type,base64Image) {
    ...
    let response = await axios.post(config.visual_url, customRequest);
    if (task_type !== '<OD>') {
...
async function UseTool(ctx, session, config, message, Tool_reply) {
  ...
  if (Tool_reply.includes("Search")) {
    ...
    if (keywords.some(keyword => message.includes(keyword))) {
      ...
      const request = createRequestBody(config, customRequest)
      //post request and get output
      let reply = await axios.post(url, request);
...
//Emoji判断
async function EmojiJudge(url, message, message2, config, session, characterName) {
    ...
    const oob = ctx.command("oob <...msg>", "与AI模型进行对话")
        ...
        .action(async ({ session, options }, ...msg) => {
            ...
            if (response.status == 200) {
                ...
                //表情包
                if (config.UseEmoji) {
                    await EmojiJudge(url, message, output, config, session, characterName);
                }
