...
async function createRequest(config, session, customConfig = {},apiMode) {
    ...
    if (api_select == "OpenAIPlus") {
        ...
        let request = Object.assign({}, defaultConfig, NewcustomConfig);
        //logger.info('完整的request:',request)
  ...
  for (const imageUrl of imageUrls) {
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
          try {
              ...
              // 验证图片数据有效性
              if (!response || !response.data || response.data.byteLength < 100) {
                  logger.error(`图片数据无效或太小: ${imageUrl}, 大小: ${response?.data?.byteLength || 0} 字节`);
              ...
              if (!base64String || base64String.length < 10) {
                  logger.error(`生成的base64字符串无效: ${base64String}`);
                  throw new Error('base64转换失败');
              }

              // 直接返回完整的base64字符串，不要截断
              base64Image = `data:image/jpeg;base64,${base64String}`;
              logger.info(`成功下载图片，大小: ${response.data.byteLength} 字节, base64长度: ${base64String.length}`);
              downloadSuccess = true;
              break;
          } catch (error) {
              logger.error(`下载图片失败 (尝试 ${attempt}/${maxAttempts}): ${imageUrl}`, error);
              await new Promise(resolve => setTimeout(resolve, retryDelay * attempt)); // 指数退避
          }
      }
      if (!downloadSuccess) {
          await session.send(`下载图片失败: ${imageUrl}`);
          base64Images.push(null); // 添加null表示下载失败
      } else {
          base64Images.push(base64Image);
      }
  }
  return base64Images.filter(img => img !== null); // 过滤掉下载失败的图片
}
...
class MessageQueue {
    ...
    async sendNext() {
        //logger.info('oob进入sendNext函数，this.queue.length：', this.queue.length);
            ...
            return new Promise(async (resolve, reject) => {
                ...
                channelTasks[gchannelId].push(async () => {
                    try {
                        ...
                        if (config.groupmessage) {
                            ...
                            if (session.channelId.includes("private")) {
                                ...
                                if (msg.includes('SystemInformation:')) {
                                    ...
                                    logger.info('oob收到content:', content)
                        ...
                        //logger.info(`usermessage多模态前:`, usermessage)
                          //原生多模态图片base64传入
                          if (config.Multimodel === true && Imagebase64 && Imagebase64.length > 0) {
                            try {
                              ...
                              // 添加所有图片
                              for (const base64Image of Imagebase64) {
                                if (base64Image && base64Image.length > 100) {
                                  ...
                                  let base64Data = formattedBase64;

                                  if (formattedBase64.startsWith('data:')) {
                                    const parts = formattedBase64.split(',');
                                  ...
                                  //logger.info(`添加图片到OpenAI格式请求，MIME类型: ${mimeType}`);
                                } else {
                                  logger.error(`无效的图片数据: ${base64Image?.length || 0} 字符`);
                                }
                              }

                              // 如果没有有效图片，则返回错误
                              if (contentArray.length <= 1) {
                                logger.error('没有有效的图片数据被添加');
                                await session.send('图片处理失败，请重新上传有效图片');
                                return;
                              }
                              ...
                              // 添加调试日志
                              logger.info(`多模态消息格式: ${JSON.stringify(contentArray[0])} 和 ${contentArray.length - 1} 张图片`);
                            } catch (error) {
                                logger.error(`处理图片数据时出错: ${error.message}`);
                                await session.send('处理图片时出错，请重试');
                                return;
                            }
                        }
                        history.push(usermessage);
                        //logger.info(`usermessage多模态后:`, usermessage)
                        ...
                        // 获取记忆表
                        if(config.memory_table && ctx.memorytable){
                          logger.info(`oob正在await ctx.memorytable.getMem`)
                            ...
                            if(trait){
                                ...
                                fullinput.forEach(entry => {
                                  if (entry.role === 'system') {
                                    ...
                                    logger.info(String(tmpContent))
                                  }
                              });
                            }else{
                              logger.info(`未回想起特征信息：因为trait为空`)
                            }
                        }else{
                            logger.info('ctx.memorytable:',ctx.memorytable)
                        }
                        ...
                        logger.info(`oob正在await createRequest`)
                        //post request and get output
                        let response;
                        try {
                            response = await createRequest(config, session, customRequest, config.multiConfig?.Talk_Post||null);

                            // 增强响应验证
                            if (!response || !response.data || !response.data.choices?.[0]?.message?.content) {
                                const errorInfo = {
                                    request: customRequest,
                                    response: response?.data || '无响应数据'
                                };
                                logger.error('API返回无效响应:', errorInfo);
                                throw new Error(`API返回无效响应: ${JSON.stringify(errorInfo)}`);
                            }
                        } catch (error) {
                            logger.error('请求模型时出错:', error);
                            await session.send('请求模型时出错，请稍后再试');
                            return;
                        }
                        ...
                        logger.info(`oob正在await sessionMap.saveHistory`)
                        await sessionMap.saveHistory(sessionId, history);
                        ...
                        //logger.info(`oob正在await sendOutput`)
                      ...
                      try {
                        logger.info('尝试下载的图片URL:', imgSrc);
                          ...
                          if (response.data && response.data.byteLength > 0) {
                              base64Image = (0, koishi_1.arrayBufferToBase64)(response.data);
                              logger.info(`图片下载成功，大小: ${response.data.byteLength} bytes`);
                              downloadSuccess = true;
                              break;
                          } else {
                              logger.warn(`图片下载返回空数据，尝试 ${attempt}/3`);
                          }
                      } catch (error) {
                          logger.error(`图片下载失败，尝试 ${attempt}/3:`, error);
                          await new Promise(resolve => setTimeout(resolve, 500));
                      }
                  }
                  if (!downloadSuccess) {
                      await session.send('下载图片失败，请重试。');
                      logger.error('所有图片下载尝试均失败');
                      return;
                  }
