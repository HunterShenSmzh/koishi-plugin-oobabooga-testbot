            ...
            return new Promise(async (resolve, reject) => {
                ...
                channelTasks[gchannelId].push(async () => {
                    try {
                        ...
                        // 获取记忆表
                        if (config.memory_table && ctx.memorytable) {
                            const { trait, memory_st, memory_lt } = await ctx.memorytable.getMem(session.userId, session.channelId.toString().replace(/-/g, ''));
                            ...
                            logger.warn(`记忆表功能已在配置中启用，但 ctx.memorytable 不可用。ctx.memorytable 的值:`, ctx.memorytable);
                        }
