...
### 用前需知
### 注意！2.0版本大改，新增的真群聊模式可能遇到各种问题，可以积极反馈！QQ讨论群：719518427
### 当前为正式版本2.0.0
...
### 推荐使用的语言模型：
chatglm3-6b：https://huggingface.co/THUDM/<br>
...
### 新版本新增：
调整了用前须知。<br>
新增：补齐了几乎所有的api高级选项。<br>
！！真群聊功能，测试中，可以通过关闭真群聊选项返回正常模式！！<br>
优化：stop string修正，模型现在不会输出一长串后再进行截段了，提高了生成效率。<br>
...
async function apply(ctx, config) {
    const oob = ctx.command("oob <message...>", "与AI模型进行对话，基于tgw本地部署")
      .action(async ({ session }, ...msg) => {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                if (config.outputMode == `extra`) {
                    ...
                    if (config.groupmessage) {
                        //真群聊
                        if (parts.length === 3 && parts[0] == encodeURIComponent(session.channelId.toString().replace(/-/g, ''))) {
                            channelId = decodeURIComponent(parts[0]);
                            characterName = decodeURIComponent(parts[1]);
                            speakerid = decodeURIComponent(parts[2].replace('.json', ''));
                            break;
                        }
                    } else {
                        //非真群聊
                    ...
                    if (config.groupmessage) {
                        //真群聊
                        if (parts.length === 2 && parts[0] == encodeURIComponent(session.channelId.toString().replace(/-/g, ''))) {
                            channelId = decodeURIComponent(parts[0]);
                            characterName = decodeURIComponent(parts[1].replace('.json', ''));
                            break;
                        }
                    } else {
                        //非真群聊
          ...
          if (config.outputMode == `extra`) {
              ...
              if (config.groupmessage) {
                  //真群聊
                  if (!channelId) {
                      sessionId = session.channelId.toString().replace(/-/g, '') + "-" + characterName + "-" + tempspeakerID;
                  } else {
                      sessionId = session.channelId.toString().replace(/-/g, '') + "-" + characterName + "-" + speakerid;
                  }
              } else {
                  //非真群聊
                  if (!channelId) {
              ...
              if (config.groupmessage) {
                  //真群聊
                  sessionId = session.channelId.toString().replace(/-/g, '') + "-" + characterName;
              } else {
                  //非真群聊
          ...
          if (config.groupmessage) {
              ...
              if (session.channelId.includes("private")) {
                  ...
                  let usernameraw = await session.bot.getGuildMember(session.guildId, session.userId);//昵称处理
                  let username = usernameraw.user.name
            ...
            if (config.outputMode == 'extra') {
                ...
                if (sessionMap.checkCharacter(character)) {
                    ...
                    if (config.groupmessage) {
                        ...
                        for (let i = 0; i < files.length; i++) {
                            let parts = files[i].split('-');
                            if (parts.length === 3 && parts[0] === encodeURIComponent(channelId)) {
                                return `已存在一个历史记录，与频道 ${channelId} 的会话对应。请不要重复创建。\n如需更换人设请先使用oob.del（别名：删除人设）指令。`;
                            }
                        }
                        let sessionId = channelId + "-" + character + "-" + config.ttsspeakerID;
                ...
                if (sessionMap.checkCharacter(character)) {
                    ...
                    if (config.groupmessage) {
                        ...
                        for (let i = 0; i < files.length; i++) {
                            let parts = files[i].split('-');
                            if (parts.length === 2 && parts[0] === encodeURIComponent(channelId)) {
                                return `已存在一个历史记录，与频道 ${channelId} 的会话对应。请不要重复创建。\n如需更换人设请先使用oob.del（别名：删除人设）指令。`;
                            }
                        }
                        let sessionId = channelId + "-" + character;
                        await sessionMap.create(sessionId);
                        if (config.select_character_notice) {
    ...
    ctx.command('oob.speaker <newspeakerid:text>', ":\n(别名：更换语音)\n更换语音角色，只在语音绑定模式生效")
        ...
        .action(async ({ session }, newspeakerid) => {
            ...
            if (config.groupmessage) {
                ...
                for (let i = 0; i < files.length; i++) {
                    ...
                    if (parts.length === 3 && parts[0] == encodeURIComponent(session.channelId.toString().replace(/-/g, ''))) {
                        channelId = decodeURIComponent(parts[0]);
                        characterName = decodeURIComponent(parts[1]);
                        oldspeakerid = decodeURIComponent(parts[2].replace('.json', ''))
                        break;
                    }
                }
            } else {
            ...
            if (!channelId) {
                ...
                if (config.groupmessage) {
                    //真群聊
                    sessionId = session.channelId.toString().replace(/-/g, '') + "-" + characterName + "-" + oldspeakerid;
                    newsessionId = session.channelId.toString().replace(/-/g, '') + "-" + characterName + "-" + newspeakerid;
                } else {
                    //非真群聊
                    sessionId = session.channelId.toString().replace(/-/g, '') + "-" + session.userId.toString() + "-" + characterName + "-" + oldspeakerid;
                    newsessionId = session.channelId.toString().replace(/-/g, '') + "-" + session.userId.toString() + "-" + characterName + "-" + newspeakerid;
                }
                ...
                if (config.groupmessage) {
                    await session.send(`已更改群聊绑定语音角色：\n channelId: ${channelId}, 人设: ${characterName}，语音角色：${newspeakerid}`);
                } else {
    ...
    ctx.command('oob.del', ":\n(别名：删除人设)\n删除当前人设")
        ...
        .action(async ({ session }) => {
            ...
            if (!channelId) {
                ...
                if (config.groupmessage) {
                    await session.send(`已删除群聊历史记录文件：\n channelId: ${channelId}, 人设: ${characterName} \n 现在可以使用oob.load（别名：加载人设）来选择新的人设了`)
                } else {
                    await session.send(`已删除历史记录文件：\n channelId: ${channelId}, userId: ${userId}, 人设: ${characterName} \n 现在可以使用oob.load（别名：加载人设）来选择新的人设了`)
                }
                return;
            }
        });
    ...
    ctx.command('oob.reset', ":\n(别名：重置人设)\n重置当前人设")
        ...
        .action(async ({ session }) => {
            ...
            if (!channelId) {
                ...
                if (config.groupmessage) {
                    await session.send(`已重置群聊历史记录文件：\n channelId: ${channelId}, 人设: ${characterName} \n 如果需要选择新的人设请使用oob.load（别名：加载人设）指令`)
                } else {
                    await session.send(`已重置历史记录文件：\n channelId: ${channelId}, userId: ${userId}, 人设: ${characterName} \n 如果需要选择新的人设请使用oob.load（别名：加载人设）指令`)
                }
                return;
            }
        });
    ...
    ctx.command('oob.check', ":\n(别名：当前人设)\n检查历史记录文件是否存在")
        ...
        .action(async ({ session }) => {
            ...
            if (config.groupmessage) {
                await session.send(`群聊文件存在：\n channelId: ${channelId}, characterName: ${characterName}`)
            } else {
                await session.send(`文件存在：\n channelId: ${channelId}, userId: ${userId}, characterName: ${characterName}`)
            }
            return;
        });
        ...
        .action(async ({ session }) => {
            ...
            if (!channelId) {
                ...
                if (history && history.length > 0) {
                    ...
                    if (config.groupmessage) {
                        await session.send(`历史记录文件为空，无法删除对话。\n channelId: ${channelId},characterName: ${characterName}`)
                    } else {
                        await session.send(`历史记录文件为空，无法删除对话。\n channelId: ${channelId}, userId: ${userId}, characterName: ${characterName}`)
                    }
                    return;
