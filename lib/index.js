...
### 新版本新增：
增加对Ollama后端的支持<br>
增加随机种自定义，防止多个同插件bot同时触发回复<br>
修复openaiPlus的自定义参数部分无法处理true false的情况<br>
5.4.1版本修复随机种忘加math.random的bug<br>
`;
...
exports.Config = koishi_1.Schema.intersect([
    ...
    koishi_1.Schema.object({
        ...
        censor_ntr_error_skip: koishi_1.Schema.boolean()
            .description('如果审核模型报错，是否跳过审核（跳过会正常回复，否则会拦截）')
            .default(true),
...
//Gemini标准格式转换
function convertOpenAIChatToGemini(customConfig) {
    let openAIMessages = customConfig.messages;
    const geminiMessages = [];
    let systemPrompt = "";

    for (const message of openAIMessages) {
        if (!message.role || !message.content) {
            console.error("Invalid input: Each message must have a 'role' and 'content'.");
            return null;
        }
        ...
        if (message.role === "user") {
            ...
            console.error(`Invalid role: ${message.role}. Only "system", "user", and "assistant" are supported.`);
            return null;
        }
    ...
    if (systemPrompt) {
        ...
        geminiMessages.unshift({
            role: "model",
            parts: [{ text: "Okay, I will follow the system instruction." }],
        });
    }
...
async function createRequest(config, session, customConfig = {},apiMode) {
    ...
    if (api_select == "OpenAI") {
        ...
        if (customConfig.continue_ == true) {
            if (customConfig.messages && customConfig.messages.length > 0) {
                lastMessage = customConfig.messages[customConfig.messages.length - 1];
                let Newmessage = convertDialogue(customConfig);
    ...
    if (api_select == "Gemini") {
        let url = `${config.apiGemini}v1beta/models/${config.Gemini_model}:generateContent?key=${config.apiKey_Gemini}`;
        ...
        if (customConfig.continue_ == true) {
            if (customConfig.messages && customConfig.messages.length > 0) {
                ...
                request = {
                    ...
                    generationConfig: {
                        stopSequences: config.Gemini_stop_sequences,
                    }
                };
            }
        } else {
            convertedMessages = convertOpenAIChatToGemini(customConfig);
            request = {
        ...
        if (config.Gemini_temperature) {
            request.generationConfig.temperature = config.Gemini_temperature;
        }
        if (config.Gemini_top_p) {
            request.generationConfig.topP = config.Gemini_top_p;
        }
        if (config.Gemini_max_tokens) {
            request.generationConfig.maxOutputTokens = config.Gemini_max_tokens;
        }
        try {
            let axiosConfig = {
                headers: {
            ...
            if (response.status == 200) {
                ...
                if (response.data.candidates && response.data.candidates[0]?.content?.parts[0]?.text) {
                    ...
                    let blockReason = response.data.promptFeedback.blockReason;
                    let blockReasonMessage = response.data.promptFeedback.blockReasonMessage;
                    session.send(`文本 "${blockReason}" 原因：${blockReasonMessage}`);
                    return null;
                }
                if (!generatedText) {
                    session.send("模型返回为空，请检查请求是否有误");
                    console.log("Gemini API Response:", response.data);
                    return null
                }
                // 拼接最后一条消息
                if (lastMessage !== '') {
                    generatedText = lastMessage.content + generatedText;
                }
                //构建openai格式的回复
                let openAIResponse = {
    ...
    if (api_select == "Ollama") {
        ...
        //传入参数
        defaultConfig.messages = customConfig.messages !== undefined ? customConfig.messages : defaultConfig.messages;
        defaultConfig.options.temperature = customConfig.temperature !== undefined ? customConfig.temperature : defaultConfig.options.temperature;
        defaultConfig.options.stop = customConfig.stop !== undefined ? customConfig.stop : defaultConfig.options.stop;
        defaultConfig.options.top_p = customConfig.top_p !== undefined ? customConfig.top_p : defaultConfig.options.top_p;
...
//ntr审查
async function censor_ntr(config, session, message) {
    ...
    if (config.censor_ntr_replace_name) {
        censor_ntr_prompt = censor_ntr_prompt + `用这个词替换相同或相似昵称：${config.censor_ntr_replace_name}。`;
    }
    character.push({ "role": "user", "content": `${censor_ntr_prompt}这里是你需要分析的聊天内容：“${message}”` });
    // 准备request
    const customRequest = {
        "messages": character,
        "temperature": 0.8,
...
// 发送output
async function sendOutput(session, config, output, ctx, speakerId) {
    let resultText = "";
            ...
            return new Promise(async (resolve, reject) => {
                ...
                channelTasks[gchannelId].push(async () => {
                    try {
                            ...
                            if (session.channelId.includes("private")) {
                                ...
                                if (msg.includes('SystemInformation:')) {
                                    ...
                                    if (config.groupmessage_withId) {
                                        content = userName + '(id:' + authID + '):' + message
                                    } else {
                                    ...
                                    if (config.censor_ntr) {
                                        if (config.censor_ntr_whitelist_id.length === 0 && config.censor_ntr_name.length === 0) {
                                            ...
                                            if (config.censor_ntr_whitelist_id.length > 0 && config.censor_ntr_whitelist_id.includes(authID)) {
                                                usermessage = { "role": "user", "content": content };
                                            } else {
                                                let censor_result = await censor_ntr(config, session, content);
                                                if (censor_result == '审查模型返回出错') {
                                                    if(censor_ntr_error_skip){
                                                        usermessage = { "role": "user", "content": content };
                                                    }else{
                                                        session.send('审查模型回复错误，请联系管理员处理此情况')
                                                        return
                                                ...
                                                if (censor_result !== '无ntr') {
                                                    ...
                                                    content = `${censor_result["替换后的聊天内容"]}`
                                                }
                                                usermessage = { "role": "user", "content": content };
                                            }
                                        }
                        ...
                        //发送output
                        await sendOutput(session, config, output, ctx, speakerId);
